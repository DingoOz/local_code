...
void show_result(const ToolCall& call, const ToolResult& res) {
    std::cout << "\033[90m" << (res.ok ? "ok" : "failed") << "\033[0m\n";

    if (call.name == "read_file" && res.ok) {
        std::cout << "\033[90m  (" << res.output.size()
                  << " bytes read)\033[0m\n";
        return;
    }
    ...
    while (pos < body.size() && shown < kMaxLines) {
        ...
        std::cout << "\033[90m  " << body.substr(pos, nl - pos) << "\033[0m\n";
        pos = nl + 1;
        ++shown;
    }
    if (pos < res.output.size() || res.output.size() > kMaxChars)
        std::cout << "\033[90m  ... [truncated]\033[0m\n";
}
...
void Agent::handle(const std::string& user_input) {
    ...
    for (int turn = 0; turn < cfg_.max_tool_turns; ++turn) {
        ...
        if (reply.find_first_not_of(" \t\r\n") == std::string::npos) {
            if (++empty_retries > 2) {
                std::cout << "\033[33m[the model went quiet — type to "
                             "continue]\033[0m\n";
                return;
            }
        ...
        if (sig == last_sig) {
            std::cout << "\033[33m[the model repeated the same "
                      << call->name
        ...
        ToolResult res = execute_tool(*call, cfg_);
    ...
    std::cout << "\033[33m[stopped after " << cfg_.max_tool_turns
